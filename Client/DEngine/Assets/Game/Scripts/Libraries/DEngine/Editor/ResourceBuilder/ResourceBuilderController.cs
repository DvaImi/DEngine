...
namespace DEngine.Editor.ResourceTools
{
    public sealed partial class ResourceBuilderController
    {
        ...
        {
            m_ConfigurationPath = DEngineType.GetConfigurationPath<ResourceBuilderConfigPathAttribute>() ?? Utility.Path.GetRegularPath(Path.Combine(Application.dataPath, "DEngine/Configs/ResourceBuilder.xml"));
            ...
            m_CompressionHelperTypeNames.AddRange(DEngineType.GetRuntimeOrEditorTypeNames(typeof(Utility.Compression.ICompressionHelper)));
            m_BuildEventHandlerTypeNames.AddRange(DEngineType.GetRuntimeOrEditorTypeNames(typeof(IBuildEventHandler)));
        ...
        public string OutputPackagePath
        {
            get
            {
                ...
                return Utility.Path.GetRegularPath(new DirectoryInfo(Utility.Text.Format("{0}/Package/{1}.{2}/", OutputDirectory, ApplicableGameVersion, InternalResourceVersion)).FullName);
            }
        }
        ...
        public string OutputFullPath
        {
            get
            {
                ...
                return Utility.Path.GetRegularPath(new DirectoryInfo(Utility.Text.Format("{0}/Full/{1}.{2}/", OutputDirectory, ApplicableGameVersion, InternalResourceVersion)).FullName);
            }
        }
        ...
        public string OutputPackedPath
        {
            get
            {
                ...
                return Utility.Path.GetRegularPath(new DirectoryInfo(Utility.Text.Format("{0}/Packed/{1}.{2}/", OutputDirectory, ApplicableGameVersion, InternalResourceVersion)).FullName);
            }
        }
        ...
        public string BuildReportPath
        {
            get
            {
                ...
                return Utility.Path.GetRegularPath(new DirectoryInfo(Utility.Text.Format("{0}/BuildReport/{1}.{2}/", OutputDirectory, ApplicableGameVersion, InternalResourceVersion)).FullName);
            }
        }
        ...
        public event DEngineAction OnLoadCompleted = null;

        public event DEngineAction<int, int> OnAnalyzingAsset = null;

        public event DEngineAction OnAnalyzeCompleted = null;

        public event DEngineFunc<string, float, bool> ProcessingAssetBundle = null;

        public event DEngineFunc<string, float, bool> ProcessingBinary = null;

        public event DEngineAction<Platform> ProcessResourceComplete = null;
        ...
        public bool BuildResources()
        {
            ...
            m_BuildReport.Initialize(BuildReportPath, ProductName, CompanyName, GameIdentifier, UnityVersion, ApplicableGameVersion, InternalResourceVersion,
            ...
            try
            {
                ...
                if (m_BuildEventHandler != null)
                {
                    ...
                    m_BuildEventHandler.OnPreprocessAllPlatforms(ProductName, CompanyName, GameIdentifier, UnityVersion, ApplicableGameVersion, InternalResourceVersion,
                ...
                if (!m_ResourceCollection.Load())
                {
                    ...
                    if (m_BuildEventHandler != null)
                    {
                        ...
                        m_BuildEventHandler.OnPostprocessAllPlatforms(ProductName, CompanyName, GameIdentifier, UnityVersion, ApplicableGameVersion, InternalResourceVersion,
                            Platforms, AssetBundleCompression, CompressionHelperTypeName, AdditionalCompressionSelected, ForceRebuildAssetBundleSelected, BuildEventHandlerTypeName, OutputDirectory, buildAssetBundleOptions,
                            WorkingPath, OutputPackageSelected, OutputPackagePath, OutputFullSelected, OutputFullPath, OutputPackedSelected, OutputPackedPath, BuildReportPath);
                    }
        ...
        private void ProcessPackageVersionList(string outputPackagePath, Platform platform)
        {
            ...
            string packageVersionListPath = Utility.Path.GetRegularPath(Path.Combine(outputPackagePath, LocalVersionListFileName));
            using (FileStream fileStream = new FileStream(packageVersionListPath, FileMode.Create, FileAccess.Write))
            {
                ...
                {
                    throw new DEngineException("Serialize package version list failure.");
                }
            }
        }
        ...
        private VersionListData ProcessUpdatableVersionList(string outputFullPath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(updatableVersionListPath, FileMode.Create, FileAccess.Write))
            {
                if (!serializer.Serialize(fileStream, versionList))
                {
                    throw new DEngineException("Serialize updatable version list failure.");
                }
            }
        ...
        private void ProcessReadOnlyVersionList(string outputPackedPath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(readOnlyVersionListPath, FileMode.Create, FileAccess.Write))
            {
                if (!serializer.Serialize(fileStream, versionList))
                {
                    throw new DEngineException("Serialize read-only version list failure.");
                }
            }
        }
