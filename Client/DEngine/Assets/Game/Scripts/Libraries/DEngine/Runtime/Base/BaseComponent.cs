...
namespace DEngine.Runtime
{
    ...
    [AddComponentMenu("DEngine/Base")]
    public sealed class BaseComponent : DEngineComponent
    {
        ...
        [SerializeField] private string m_TextHelperTypeName = "DEngine.Runtime.DefaultTextHelper";

        [SerializeField] private string m_VersionHelperTypeName = "DEngine.Runtime.DefaultVersionHelper";

        [SerializeField] private string m_LogHelperTypeName = "DEngine.Runtime.DefaultLogHelper";

        [SerializeField] private string m_CompressionHelperTypeName = "DEngine.Runtime.DefaultCompressionHelper";

        [SerializeField] private string m_JsonHelperTypeName = "DEngine.Runtime.DefaultJsonHelper";
        ...
        private void Update()
        {
            DEngineEntry.Update(Time.deltaTime, Time.unscaledDeltaTime);
        }
        ...
        private void InitVersionHelper()
        {
            ...
            {
                throw new DEngineException(Utility.Text.Format("Can not find version helper type '{0}'.", m_VersionHelperTypeName));
            }

            Version.IVersionHelper versionHelper = (DEngine.Version.IVersionHelper)Activator.CreateInstance(versionHelperType);
            if (versionHelper == null)
            {
                throw new DEngineException(Utility.Text.Format("Can not create version helper instance '{0}'.", m_VersionHelperTypeName));
            }
        ...
        private void InitLogHelper()
        {
            ...
            {
                throw new DEngineException(Utility.Text.Format("Can not find log helper type '{0}'.", m_LogHelperTypeName));
            }

            DEngineLog.ILogHelper logHelper = (DEngineLog.ILogHelper)Activator.CreateInstance(logHelperType);
            if (logHelper == null)
            {
                throw new DEngineException(Utility.Text.Format("Can not create log helper instance '{0}'.", m_LogHelperTypeName));
            }

            DEngineLog.SetLogHelper(logHelper);
        }
