...
namespace DEngine.Resource
{
    internal sealed partial class ResourceManager : DEngineModule, IResourceManager
    {
        ...
        private sealed class ResourceIniter
        {
            ...
            public DEngineAction ResourceInitComplete;
            ...
            public void InitResources(string currentVariant)
            {
                ...
                m_ResourceManager.m_ResourceHelper.LoadBytes(Utility.Path.GetRemotePath(Path.Combine(m_ResourceManager.m_ReadOnlyPath, LocalVersionListFileName)), new LoadBytesCallbacks(OnLoadPackageVersionListSuccess, OnLoadPackageVersionListFailure), null);
            }
            ...
            private void OnLoadPackageVersionListSuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                try
                {
                    ...
                    {
                        throw new DEngineException("Deserialize package version list failure.");
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new DEngineException(Utility.Text.Format("Parse package version list exception '{0}'.", exception), exception);
                }
                finally
                {
            ...
            {
                throw new DEngineException(Utility.Text.Format("Package version list '{0}' is invalid, error message is '{1}'.", fileUri, string.IsNullOrEmpty(errorMessage) ? "<Empty>" : errorMessage));
            }
        }
    }
}
