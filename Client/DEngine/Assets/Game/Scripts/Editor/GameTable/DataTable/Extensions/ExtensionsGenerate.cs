...
namespace Game.Editor.DataTableTools
{
    public static class ExtensionsGenerate
    {
        ...
        private static void GenerateDataTableExtensionArray(IDictionary<string, DataTableProcessor.DataProcessor> dataProcessors)
        {
            ...
            AddNameSpaces(sb);
            sb.AppendLine($"namespace {DataTableSetting.Instance.NameSpace}");
            sb.AppendLine("{");
            ...
            foreach (var item in dataProcessors)
            {
                ...
                {
                    sb.AppendLine($"\t\tpublic static {item.Value.LanguageKeyword}[] Parse{item.Value.LanguageKeyword.Replace(".", "")}Array(string value)");
                }
                else
                {
                ...
                if (item.Value.IsEnum)
                {
                    sb.AppendLine($"\t\t\t{item.Value.LanguageKeyword}[] array = new {item.Value.LanguageKeyword}[splitValue.Length];");
                }
                else
                {
        ...
        private static void GenerateDataTableExtensionList(IDictionary<string, DataTableProcessor.DataProcessor> dataProcessors)
        {
            ...
            foreach (var item in dataProcessors)
            {
                if (item.Value.IsEnum)
                {
                    sb.AppendLine($"\t\tpublic static List<{item.Value.LanguageKeyword}> Parse{item.Value.LanguageKeyword.Replace(".", "")}List(string value)");
                }
                else
                {
                ...
                if (item.Value.IsEnum)
                {
                    sb.AppendLine($"\t\t\tList<{item.Value.LanguageKeyword}> list = new List<{item.Value.LanguageKeyword}>(splitValue.Length);");
                }
                else
                {
        ...
        private static void GenerateBinaryReaderExtensionList(IDictionary<string, DataTableProcessor.DataProcessor> dataProcessors)
        {
            ...
            foreach (var item in dataProcessors)
            {
                if (item.Value.IsEnum)
                {
                    sb.AppendLine($"\t\tpublic static List<{item.Value.LanguageKeyword}> Read{item.Value.LanguageKeyword.Replace(".", "")}List(this BinaryReader binaryReader)");
                }
                else
                {
                ...
                if (item.Value.IsEnum)
                {
                    sb.AppendLine($"\t\t\tList<{item.Value.LanguageKeyword}> list = new List<{item.Value.LanguageKeyword}>(count);");
                }
                else
                {
                ...
                else
                {
                    ...
                    {
                        sb.AppendLine($"\t\t\t\tlist.Add(({item.Value.LanguageKeyword})binaryReader.Read7BitEncodedInt32());");
                    }
                    else
                    {
        ...
        private static void GenerateBinaryReaderExtensionArray(IDictionary<string, DataTableProcessor.DataProcessor> dataProcessors)
        {
            ...
            foreach (var item in dataProcessors)
            {
                if (item.Value.IsEnum)
                {
                    sb.AppendLine($"\t\tpublic static {item.Value.LanguageKeyword}[] Read{item.Value.LanguageKeyword.Replace(".", "")}Array(this BinaryReader binaryReader)");
                }
                else
                {
                ...
                if (item.Value.IsEnum)
                {
                    sb.AppendLine($"\t\t\t{item.Value.LanguageKeyword}[] array = new {item.Value.LanguageKeyword}[count];");
                }
                else
                {
                ...
                else
                {
                    ...
                    {
                        sb.AppendLine($"\t\t\t\tarray[i] = ({item.Value.LanguageKeyword})binaryReader.Read7BitEncodedInt32();");
                    }
                    else
                    {
        ...
        private static void GenerateDataTableExtensionDictionary(List<DataTableProcessor.DataProcessor[]> keyValueList)
        {
            ...
            foreach (var item in keyValueList)
            {
                ...
                sb.AppendLine($"\t\tpublic static Dictionary<{dataProcessorT1.LanguageKeyword},{dataProcessorT2.LanguageKeyword}> Parse{names.Item1}{names.Item2}Dictionary(string value)");
                ...
                sb.AppendLine("\t\t\t}");
                sb.AppendLine("\t\t\tstring[] splitValue = value.Split('|');");
                sb.AppendLine($"\t\t\tDictionary<{dataProcessorT1.LanguageKeyword},{dataProcessorT2.LanguageKeyword}> dictionary = new Dictionary<{dataProcessorT1.LanguageKeyword},{dataProcessorT2.LanguageKeyword}>(splitValue.Length);");
                sb.AppendLine("\t\t\tfor (int i = 0; i < splitValue.Length; i++)");
                sb.AppendLine("\t\t\t{");
                ...
                if (dataProcessorT1.IsSystem)
                {
                    if (dataProcessorT2.IsSystem)
                    {
                        ...
                        {
                            sb.AppendLine("\t\t\t\tdictionary.Add(keyValue[0].Substring(1),keyValue[1].Substring(0, keyValue[1].Length - 1));");
                        }
                        else if (dataProcessorT1.LanguageKeyword == "string")
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(keyValue[0].Substring(1),{dataProcessorT2.Type.Name}.Parse(keyValue[1].Substring(0, keyValue[1].Length - 1)));");
                        }
                        else if (dataProcessorT2.LanguageKeyword == "string")
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add({dataProcessorT1.Type.Name}.Parse(keyValue[0].Substring(1)),keyValue[1].Substring(0, keyValue[1].Length - 1));");
                        }
                        else
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add({dataProcessorT1.Type.Name}.Parse(keyValue[0].Substring(1)),{dataProcessorT2.Type.Name}.Parse(keyValue[1].Substring(0, keyValue[1].Length - 1)));");
                        }
                    }
                    else
                    {
                        if (dataProcessorT1.LanguageKeyword == "string")
                            ...
                            {
                                sb.AppendLine($"\t\t\t\tdictionary.Add(keyValue[0].Substring(1),EnumParse<{dataProcessorT2.LanguageKeyword}>(keyValue[1].Substring(0, keyValue[1].Length - 1)));");
                            }
                            else
                            {
                                sb.AppendLine($"\t\t\t\tdictionary.Add(keyValue[0].Substring(1),Parse{dataProcessorT2.Type.Name}(keyValue[1].Substring(0, keyValue[1].Length - 1)));");
                            }
                        else if (dataProcessorT2.IsEnum)
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add({dataProcessorT1.Type.Name}.Parse(keyValue[0].Substring(1)),EnumParse<{dataProcessorT2.LanguageKeyword}>(keyValue[1].Substring(0, keyValue[1].Length - 1)));");
                        }
                        else
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add({dataProcessorT1.Type.Name}.Parse(keyValue[0].Substring(1)),Parse{dataProcessorT2.Type.Name}(keyValue[1].Substring(0, keyValue[1].Length - 1)));");
                        }
                    }
                }
                else
                {
                    if (dataProcessorT2.IsSystem)
                    {
                        if (dataProcessorT2.LanguageKeyword == "string")
                            ...
                            {
                                sb.AppendLine($"\t\t\t\tdictionary.Add(EnumParse<{dataProcessorT1.LanguageKeyword}>(keyValue[0].Substring(1)),keyValue[1].Substring(0, keyValue[1].Length - 1));");
                            }
                            else
                            {
                                sb.AppendLine($"\t\t\t\tdictionary.Add(Parse{dataProcessorT1.Type.Name}(keyValue[0].Substring(1)),keyValue[1].Substring(0, keyValue[1].Length - 1));");
                            }
                        else if (dataProcessorT1.IsEnum)
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(EnumParse<{dataProcessorT1.LanguageKeyword}>(keyValue[0].Substring(1)),{dataProcessorT2.Type.Name}.Parse(keyValue[1].Substring(0, keyValue[1].Length - 1)));");
                        }
                        else
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(Parse{dataProcessorT1.Type.Name}(keyValue[0].Substring(1)),{dataProcessorT2.Type.Name}.Parse(keyValue[1].Substring(0, keyValue[1].Length - 1)));");
                        }
                    }
                    else
                    {
                        if (dataProcessorT2.IsEnum)
                        {
                            sb.AppendLine(dataProcessorT1.IsEnum ? $"\t\t\t\tdictionary.Add(EnumParse<{dataProcessorT1.LanguageKeyword}>(keyValue[1].Substring(0, keyValue[1].Length - 1)),EnumParse<{dataProcessorT2.LanguageKeyword}>(keyValue[1].Substring(0, keyValue[1].Length - 1)));" : $"\t\t\t\tdictionary.Add(Parse{dataProcessorT1.Type.Name}(keyValue[0].Substring(1)),EnumParse<{dataProcessorT2.LanguageKeyword}>(keyValue[1].Substring(0, keyValue[1].Length - 1)));");
                        }
                        else if (dataProcessorT1.IsEnum)
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(EnumParse<{dataProcessorT2.LanguageKeyword}>(keyValue[1].Substring(0, keyValue[1].Length - 1)),Parse{dataProcessorT2.Type.Name}(keyValue[0].Substring(1)));");
                        }
                        else
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(Parse{dataProcessorT1.Type.Name}(keyValue[0].Substring(1)),Parse{dataProcessorT2.Type.Name}(keyValue[1].Substring(0, keyValue[1].Length - 1)));");
                        }
                    }
                }
        ...
        private static void GenerateBinaryReaderExtensionDictionary(List<DataTableProcessor.DataProcessor[]> keyValueList)
        {
            var sb = new StringBuilder();
            AddNameSpaces(sb);
            ...
            foreach (var item in keyValueList)
            {
                ...
                var dataProcessorT2 = item[1];
                (string, string) names = GetNames(dataProcessorT1, dataProcessorT2);
                sb.AppendLine($"\t\tpublic static Dictionary<{dataProcessorT1.LanguageKeyword},{dataProcessorT2.LanguageKeyword}> Read{names.Item1}{names.Item2}Dictionary(this BinaryReader binaryReader)");
                sb.AppendLine("\t\t{");
                sb.AppendLine("\t\t\tint count = binaryReader.Read7BitEncodedInt32();");
                sb.AppendLine($"\t\t\tDictionary<{dataProcessorT1.LanguageKeyword},{dataProcessorT2.LanguageKeyword}> dictionary = new Dictionary<{dataProcessorT1.LanguageKeyword},{dataProcessorT2.LanguageKeyword}>(count);");
                sb.AppendLine("\t\t\tfor (int i = 0; i < count; i++)");
                sb.AppendLine("\t\t\t{");
                ...
                if (dataProcessorT1.IsSystem && dataProcessorT1.Type != typeof(DateTime))
                {
                    if (dataProcessorT2.IsSystem && dataProcessorT2.Type != typeof(DateTime))
                    {
                        if (t1LanguageKeyword == "int" || t1LanguageKeyword == "uint" || t1LanguageKeyword == "long" || t1LanguageKeyword == "ulong")
                        {
                            ...
                            {
                                sb.AppendLine($"\t\t\t\tdictionary.Add(binaryReader.Read7BitEncoded{dataProcessorT1.Type.Name}(),binaryReader.Read7BitEncoded{dataProcessorT2.Type.Name}());");
                            }
                            else
                            {
                                sb.AppendLine($"\t\t\t\tdictionary.Add(binaryReader.Read7BitEncoded{dataProcessorT1.Type.Name}(),binaryReader.Read{dataProcessorT2.Type.Name}());");
                            }
                        }
                        else
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(binaryReader.Read{dataProcessorT1.Type.Name}(),binaryReader.Read{dataProcessorT2.Type.Name}());");
                        }
                    }
                    else
                    {
                        if (t1LanguageKeyword == "int" || t1LanguageKeyword == "uint" || t1LanguageKeyword == "long" || t1LanguageKeyword == "ulong")
                        {
                            if (dataProcessorT2.IsEnum)
                            {
                                sb.AppendLine($"\t\t\t\tdictionary.Add(binaryReader.Read7BitEncoded{dataProcessorT1.Type.Name}(),({dataProcessorT2.LanguageKeyword}) binaryReader.Read7BitEncodedInt32());");
                            }
                            else
                            {
                                sb.AppendLine($"\t\t\t\tdictionary.Add(binaryReader.Read7BitEncoded{dataProcessorT1.Type.Name}(), Read{dataProcessorT2.LanguageKeyword}(binaryReader));");
                            }
                        }
                        else if (dataProcessorT2.IsEnum)
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(binaryReader.Read{dataProcessorT1.Type.Name}(),({dataProcessorT2.LanguageKeyword}) binaryReader.Read7BitEncodedInt32());");
                        }
                        else
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(binaryReader.Read{dataProcessorT1.Type.Name}(),Read{dataProcessorT2.LanguageKeyword}(binaryReader));");
                        }
                    }
                }
                else
                {
                    if (dataProcessorT2.IsSystem && dataProcessorT2.Type != typeof(DateTime))
                    {
                        if (t2LanguageKeyword == "int" || t2LanguageKeyword == "uint" || t2LanguageKeyword == "long" || t2LanguageKeyword == "ulong")
                        {
                            if (dataProcessorT1.IsEnum)
                            {
                                sb.AppendLine($"\t\t\t\tdictionary.Add(({dataProcessorT1.LanguageKeyword}) binaryReader.Read7BitEncodedInt32(),({dataProcessorT2.LanguageKeyword}) binaryReader.Read7BitEncodedInt32());");
                            }
                            else
                            {
                                sb.AppendLine($"\t\t\t\tdictionary.Add(Read{dataProcessorT1.LanguageKeyword}(binaryReader),binaryReader.Read7BitEncoded{dataProcessorT2.Type.Name}());");
                            }
                        }

                        else if (dataProcessorT1.IsEnum)
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(({dataProcessorT1.LanguageKeyword}) binaryReader.Read7BitEncodedInt32(),binaryReader.Read{dataProcessorT2.Type.Name}());");
                        }
                        else
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(Read{dataProcessorT1.LanguageKeyword}(binaryReader),binaryReader.Read{dataProcessorT2.Type.Name}());");
                        }
                    }
                    else
                    {
                        if (dataProcessorT2.IsEnum)
                        {
                            sb.AppendLine(dataProcessorT1.IsEnum ? $"\t\t\t\tdictionary.Add(({dataProcessorT1.LanguageKeyword}) binaryReader.Read7BitEncodedInt32(),({dataProcessorT2.LanguageKeyword}) binaryReader.Read7BitEncodedInt32());" : $"\t\t\t\tdictionary.Add(Read{dataProcessorT1.LanguageKeyword}(binaryReader),({dataProcessorT2.LanguageKeyword}) binaryReader.Read7BitEncodedInt32());");
                        }
                        else if (dataProcessorT1.IsEnum)
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(({dataProcessorT1.LanguageKeyword}) binaryReader.Read7BitEncodedInt32(),Read{dataProcessorT2.LanguageKeyword}(binaryReader));");
                        }
                        else
                        {
                            sb.AppendLine($"\t\t\t\tdictionary.Add(Read{dataProcessorT1.LanguageKeyword}(binaryReader),Read{dataProcessorT2.LanguageKeyword}(binaryReader));");
                        }
                    }
                }
