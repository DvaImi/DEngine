...
namespace Game.Editor
{
    public class EditorIcons : EditorWindow
    {
...
#endif
                if (GUILayout.Button(EditorGUIUtility.IconContent("winbtn_mac_close_h"), GUILayout.Width(22)))
                {
                    m_Search = "";
                }
            }
        ...
        private bool DoSearch => !string.IsNullOrWhiteSpace(m_Search) && m_Search != "";
        ...
        private void SaveAllIcons()
        {
            ...
            try
            {
                foreach (string icon in s_Ico_List)
                {
        ...
        private void OnEnable()
        {
            ...
            var allIcons = s_Ico_List.Where(x => GetIcon(x) != null);
            ...
            s_Ico_List = s_Ico_List.ToList().Concat(unique).ToArray();
        ...
        private void OnGUI()
        {
            ...
            using (new GUILayout.HorizontalScope(EditorStyles.toolbar))
            {
                ...
                s_ViewBigIcons = GUILayout.SelectionGrid(
                    s_ViewBigIcons ? 1 : 0, new string[] { "Small", "Big" },
            ...
            using (var scope = new GUILayout.ScrollViewScope(m_Scroll))
            {
                ...
                m_ButtonSize = s_ViewBigIcons ? 70 : 40;
                ...
                else iconList = s_ViewBigIcons ? iconContentListBig : iconContentListSmall;
            ...
            using (new GUILayout.HorizontalScope(EditorStyles.helpBox, GUILayout.MaxHeight(s_ViewBigIcons ? 140 : 120)))
            {
                using (new GUILayout.VerticalScope(GUILayout.Width(130)))
                {
                    ...
                    GUILayout.Button(iconSelected,
                        s_DarkPreview ? iconPreviewBlack : iconPreviewWhite,
                        GUILayout.Width(128), GUILayout.Height(s_ViewBigIcons ? 128 : 40));

                    GUILayout.Space(5);

                    s_DarkPreview = GUILayout.SelectionGrid(
                        s_DarkPreview ? 1 : 0, new string[] { "Light", "Dark" },
        ...
        private void InitIcons()
        {
            ...
            for (var i = 0; i < s_Ico_List.Length; ++i)
            {
                GUIContent ico = GetIcon(s_Ico_List[i]);

                if (ico == null)
                {
                    iconMissingNames.Add(s_Ico_List[i]);
                    continue;
                }

                ico.tooltip = s_Ico_List[i];
        ...
        private static string[] s_Ico_List =
        {
