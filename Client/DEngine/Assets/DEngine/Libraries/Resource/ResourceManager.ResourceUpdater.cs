...
namespace DEngine.Resource
{
    internal sealed partial class ResourceManager : DEngineModule, IResourceManager
    {
        ...
        private sealed partial class ResourceUpdater
        {
            ...
            public DEngineAction<string, int, long> ResourceApplyStart;
            public DEngineAction<ResourceName, string, string, int, int> ResourceApplySuccess;
            public DEngineAction<ResourceName, string, string> ResourceApplyFailure;
            public DEngineAction<string, bool> ResourceApplyComplete;
            public DEngineAction<ResourceName, string, string, int, int, int> ResourceUpdateStart;
            public DEngineAction<ResourceName, string, string, int, int> ResourceUpdateChanged;
            public DEngineAction<ResourceName, string, string, int, int> ResourceUpdateSuccess;
            public DEngineAction<ResourceName, string, int, int, string> ResourceUpdateFailure;
            public DEngineAction<ResourceGroup, bool> ResourceUpdateComplete;
            public DEngineAction ResourceUpdateAllComplete;
            ...
            public void ApplyResources(string resourcePackPath)
            {
                ...
                {
                    throw new DEngineException("You must check resources complete first.");
                }

                if (m_ApplyingResourcePackStream != null)
                {
                    throw new DEngineException(Utility.Text.Format("There is already a resource pack '{0}' being applied.", m_ApplyingResourcePackPath));
                }

                if (m_UpdatingResourceGroup != null)
                {
                    throw new DEngineException(Utility.Text.Format("There is already a resource group '{0}' being updated.", m_UpdatingResourceGroup.Name));
                }

                if (m_UpdateWaitingInfoWhilePlaying.Count > 0)
                {
                    throw new DEngineException("There are already some resources being updated while playing.");
                }
                ...
                try
                {
                    ...
                    {
                        throw new DEngineException("Deserialize resource pack version list failure.");
                    }

                    if (versionList.Offset + versionList.Length != length)
                    {
                        throw new DEngineException("Resource pack length is invalid.");
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new DEngineException(Utility.Text.Format("Apply resources '{0}' with exception '{1}'.", resourcePackPath, exception), exception);
                }
            }
            ...
            public void UpdateResources(ResourceGroup resourceGroup)
            {
                if (m_DownloadManager == null)
                {
                    throw new DEngineException("You must set download manager first.");
                }
            ...
            public void StopUpdateResources()
            {
                ...
                if (m_UpdatingResourceGroup == null)
                {
                    throw new DEngineException("There is no resource group being updated.");
                }
            ...
            private void GenerateReadWriteVersionList()
            {
                ...
                try
                {
                    ...
                    {
                        throw new DEngineException("Serialize read-write version list failure.");
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new DEngineException(Utility.Text.Format("Generate read-write version list exception '{0}'.", exception), exception);
                }
            ...
            private void OnDownloadStart(object sender, DownloadStartEventArgs e)
            {
                ...
                {
                    throw new DEngineException(Utility.Text.Format("File '{0}' is too large.", e.DownloadPath));
                }
