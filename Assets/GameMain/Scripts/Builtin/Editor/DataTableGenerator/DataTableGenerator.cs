...
namespace GeminiLion.Editor.DataTableTools
{
    public sealed class DataTableGenerator
    {
        ...
        {
            return new DataTableProcessor(Utility.Path.GetRegularPath(Path.Combine(GeminiLionSetting.Instance.DataTablePath, dataTableName + ".txt")), Encoding.GetEncoding("UTF-8"), 1, 2, null, 3, 4, 1);
        }
        ...
        {
            string binaryDataFileName = Utility.Path.GetRegularPath(Path.Combine(GeminiLionSetting.Instance.DataTablePath, dataTableName + ".bytes"));
        ...
        {
            dataTableProcessor.SetCodeTemplate(GeminiLionSetting.Instance.CSharpCodeTemplateFileName, Encoding.UTF8);
            dataTableProcessor.SetCodeGenerator(DataTableCodeGenerator);

            string csharpCodeFileName = Utility.Path.GetRegularPath(Path.Combine(GeminiLionSetting.Instance.CSharpCodePath, "DR" + dataTableName + ".cs"));
        ...
        private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
        {
            ...
            codeContent.Replace("__DATA_TABLE_NAME_SPACE__", GeminiLionSetting.Instance.HotfixNameSpace);
