...
namespace GeminiLion.Editor
{
    public class HybridCLRBuilderController
    {
        ...
        {
            IOUtility.CreateDirectoryIfNotExists(GeminiLionSetting.Instance.HotfixDllPath);

            // Copy Hotfix Dll
            string oriFileName = Path.Combine(SettingsUtil.GetHotUpdateDllsOutputDirByTarget(buildTarget), GeminiLionSetting.Instance.HotfixDllNameMain);
            string desFileName = Path.Combine(GeminiLionSetting.Instance.HotfixDllPath, GeminiLionSetting.Instance.HotfixDllNameMain + GeminiLionSetting.Instance.HotfixDllSuffix);
            File.Copy(oriFileName, desFileName, true);

            // Copy AOT Dll
            string aotDllPath = SettingsUtil.GetAssembliesPostIl2CppStripDir(buildTarget);
            foreach (var dllName in GeminiLionSetting.Instance.AOTDllNames)
            {
                ...
                desFileName = Path.Combine(GeminiLionSetting.Instance.HotfixDllPath, dllName + GeminiLionSetting.Instance.HotfixDllSuffix);
                File.Copy(oriFileName, desFileName, true);
            }
