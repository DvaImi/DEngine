...
namespace GeminiLion.Editor
{
    public static class PreloadUtility
    {
        ...
        {
            using (FileStream stream = new(GeminiLionSetting.Instance.PreloadInfoPath, FileMode.Create, FileAccess.Write))
            {
        ...
        {
            if (!File.Exists(GeminiLionSetting.Instance.PreloadInfoPath))
            {
                PreloadInfo newPreloadInfo = new();
                SavePreloadInfoToJson(JsonConvert.SerializeObject(newPreloadInfo));
                return newPreloadInfo;
            }
            string preloadInfoJson = File.ReadAllText(GeminiLionSetting.Instance.PreloadInfoPath);
