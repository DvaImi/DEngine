...
namespace GeminiLion.Editor
{
    ...
    public class EntityAndUIFormCodeGenerator : EditorWindow
    {
        ...
        [MenuItem("GeminiLion/Code Generator/EntityAndUIForm")]
        ...
        private void OnGUI()
        {
            ...
            switch (m_GenCodeType)
            {
                case GenCodeType.Entity:
                    EditorGUILayout.LabelField(m_IsHotfix ? GeminiLionSetting.Instance.HotfixEntityCodePath : GeminiLionSetting.Instance.EntityCodePath);
                    break;
                case GenCodeType.UIForm:
                    EditorGUILayout.LabelField(m_IsHotfix ? GeminiLionSetting.Instance.HotfixUIFormCodePath : GeminiLionSetting.Instance.UIFormCodePath);
                    break;
            }
        ...
        {
            //根据是否为热更新实体来决定一些参数
            string codepath = m_IsHotfix ? GeminiLionSetting.Instance.HotfixEntityCodePath : GeminiLionSetting.Instance.EntityCodePath;
            string nameSpace = m_IsHotfix ? GeminiLionSetting.Instance.HotfixNameSpace : GeminiLionSetting.Instance.NameSpace;
        ...
        {
            //根据是否为热更新界面来决定一些参数
            string codepath = m_IsHotfix ? GeminiLionSetting.Instance.HotfixUIFormCodePath : GeminiLionSetting.Instance.UIFormCodePath;
            string nameSpace = m_IsHotfix ? GeminiLionSetting.Instance.HotfixNameSpace : GeminiLionSetting.Instance.NameSpace;
        ...
        private void GenEntityMainLogicCode(string codePath, GameObject go, string nameSpace, string logicBaseClass)
        {
            ...
            using (StreamWriter sw = new StreamWriter($"{codePath}/EntityLogic/{go.name}.cs"))
            {
                ...
                //OnHide方法 归还实体数据引用
                sw.WriteLine($"\t\t{accessModifier} override void OnHide(bool isShutdown,object userData)");
                sw.WriteLine("\t\t{");
        ...
        private void GenShowEntityCode(string codePath, GameObject go, string nameSpace)
        {
                   ...
                   new StreamWriter($"{codePath}/ShowEntityExtension/ShowEntityExtension.Show{go.name}.cs"))
            {
                ...
                {
                    sw.WriteLine(
                        "\t\t\tGeminiLion.HotfixEntityData tData = GameFramework.ReferencePool.Acquire<GeminiLion.HotfixEntityData>();");
                    sw.WriteLine($"\t\t\ttData.Fill(data.Id,data.TypeId);");
                    sw.WriteLine("\t\t\ttData.Position = data.Position;");
                    sw.WriteLine("\t\t\ttData.Rotation = data.Rotation;");
                    sw.WriteLine("");
                    sw.WriteLine($"\t\t\tentityComponent.ShowHotfixEntity(typeof({go.name}Logic),0, tData);");
                }
                else
                {
                ...
                if (m_IsHotfix)
                {
                    ...
                    sw.WriteLine("\t\t\ttData.Rotation = data.Rotation;");
                    sw.WriteLine("");
                    sw.WriteLine($"\t\t\tEntity entity = await entityComponent.AwaitShowEntity(typeof({go.name}Logic),0, tData);");
                }
                else
                {
                    sw.WriteLine(
