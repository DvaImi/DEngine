...
namespace DEngine.Runtime
{
    ...
    public sealed class AndroidFileSystemStream : FileSystemStream
    {
        ...
        static AndroidFileSystemStream()
        {
            ...
            if (unityPlayer == null)
            {
                throw new DEngineException("Unity player is invalid.");
            }

            AndroidJavaObject currentActivity = unityPlayer.GetStatic<AndroidJavaObject>("currentActivity");
            if (currentActivity == null)
            {
                throw new DEngineException("Current activity is invalid.");
            }

            AndroidJavaObject assetManager = currentActivity.Call<AndroidJavaObject>("getAssets");
            if (assetManager == null)
            {
                throw new DEngineException("Asset manager is invalid.");
            }
        ...
        public AndroidFileSystemStream(string fullPath, FileSystemAccess access, bool createNew)
        {
            ...
            {
                throw new DEngineException(Utility.Text.Format("'{0}' is not supported in AndroidFileSystemStream.", access));
            }

            if (createNew)
            {
                throw new DEngineException("Create new is not supported in AndroidFileSystemStream.");
            }

            int position = fullPath.LastIndexOf(SplitFlag, StringComparison.Ordinal);
            if (position < 0)
            {
                throw new DEngineException("Can not find split flag in full path.");
            }
            ...
            {
                throw new DEngineException(Utility.Text.Format("Open file '{0}' from Android asset manager failure.", fullPath));
            }
        ...
        protected override long Position
        {
            get
            {
                throw new DEngineException("Get position is not supported in AndroidFileSystemStream.");
            }
            set
            {
        ...
        {
            throw new DEngineException("SetLength is not supported in AndroidFileSystemStream.");
        }
        ...
        {
            throw new DEngineException("WriteByte is not supported in AndroidFileSystemStream.");
        }
        ...
        {
            throw new DEngineException("Write is not supported in AndroidFileSystemStream.");
        }
        ...
        {
            throw new DEngineException("Flush is not supported in AndroidFileSystemStream.");
        }
