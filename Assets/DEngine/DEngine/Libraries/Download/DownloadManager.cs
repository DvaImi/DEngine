...
namespace DEngine.Download
{
    ...
    internal sealed partial class DownloadManager : DEngineModule, IDownloadManager
    {
        ...
        public int AddDownload(string downloadPath, string downloadUri, string tag, int priority, object userData)
        {
            ...
            {
                throw new DEngineException("Download path is invalid.");
            }

            if (string.IsNullOrEmpty(downloadUri))
            {
                throw new DEngineException("Download uri is invalid.");
            }

            if (TotalAgentCount <= 0)
            {
                throw new DEngineException("You must add download agent first.");
            }
