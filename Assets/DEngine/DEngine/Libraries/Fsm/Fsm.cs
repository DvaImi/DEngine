...
namespace DEngine.Fsm
{
    ...
    internal sealed class Fsm<T> : FsmBase, IReference, IFsm<T> where T : class
    {
        ...
        public static Fsm<T> Create(string name, T owner, params FsmState<T>[] states)
        {
            if (owner == null)
            {
                throw new DEngineException("FSM owner is invalid.");
            }

            if (states == null || states.Length < 1)
            {
                throw new DEngineException("FSM states is invalid.");
            }
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                {
                    throw new DEngineException(Utility.Text.Format("FSM '{0}' state '{1}' is already exist.", new TypeNamePair(typeof(T), name), stateType.FullName));
                }
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            if (IsRunning)
            {
                throw new DEngineException("FSM is running, can not start again.");
            }

            FsmState<T> state = GetState<TState>();
            if (state == null)
            {
                throw new DEngineException(Utility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), typeof(TState).FullName));
            }
        ...
        public void Start(Type stateType)
        {
            ...
            if (stateType == null)
            {
                throw new DEngineException("State type is invalid.");
            }

            if (!typeof(FsmState<T>).IsAssignableFrom(stateType))
            {
                throw new DEngineException(Utility.Text.Format("State type '{0}' is invalid.", stateType.FullName));
            }

            FsmState<T> state = GetState(stateType);
            if (state == null)
            {
                throw new DEngineException(Utility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), stateType.FullName));
            }
        ...
        public bool HasData(string name)
        {
            ...
            {
                throw new DEngineException("Data name is invalid.");
            }
        ...
        internal void ChangeState(Type stateType)
        {
            if (m_CurrentState == null)
            {
                throw new DEngineException("Current state is invalid.");
            }

            FsmState<T> state = GetState(stateType);
            if (state == null)
            {
                throw new DEngineException(Utility.Text.Format("FSM '{0}' can not change state to '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), stateType.FullName));
            }
