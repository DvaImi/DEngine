    ...
    internal sealed partial class ResourceManager : DEngineModule, IResourceManager
    {
        ...
        private sealed class ResourceGroupCollection : IResourceGroupCollection
        {
            ...
            public ResourceGroupCollection(ResourceGroup[] resourceGroups, Dictionary<ResourceName, ResourceInfo> resourceInfos)
            {
                ...
                {
                    throw new DEngineException("Resource groups is invalid.");
                }

                if (resourceInfos == null)
                {
                    throw new DEngineException("Resource infos is invalid.");
                }
                ...
                for (int i = 0; i < lastIndex; i++)
                {
                    ...
                    {
                        throw new DEngineException(Utility.Text.Format("Resource group index '{0}' is invalid.", i));
                    }
                    ...
                    for (int j = i + 1; j < resourceGroups.Length; j++)
                    {
                        ...
                        {
                            throw new DEngineException(Utility.Text.Format("Resource group '{0}' duplicated.", resourceGroups[i].Name));
                        }
                    }
                }
                ...
                {
                    throw new DEngineException(Utility.Text.Format("Resource group index '{0}' is invalid.", lastIndex));
                }
                ...
                foreach (ResourceGroup resourceGroup in m_ResourceGroups)
                {
                    ...
                    foreach (ResourceName resourceName in cachedResourceNames)
                    {
                        ...
                        {
                            throw new DEngineException(Utility.Text.Format("Resource info '{0}' is invalid.", resourceName.FullName));
                        }
