namespace DEngine.Download
{
    internal sealed partial class DownloadManager : DEngineModule, IDownloadManager
    {
        ...
        {
            private readonly DEngineLinkedList<DownloadCounterNode> m_DownloadCounterNodes;
            ...
            public DownloadCounter(float updateInterval, float recordInterval)
            {
                ...
                {
                    throw new DEngineException("Update interval is invalid.");
                }

                if (recordInterval <= 0f)
                {
                    throw new DEngineException("Record interval is invalid.");
                }

                m_DownloadCounterNodes = new DEngineLinkedList<DownloadCounterNode>();
